...
while True:
    print(page)
    respons = requests.get(f"https://www.ceneo.pl/91715703#tab=reviews{page}", allow_redirects=False)
    if respons.status_code==200:
        ...
        for opinion in opinions:
            ...
            try:
                ...
                verified = None
            post_date = opinion.select("span.user-post__published > time:nth-child(1)").pop(0)['datetime']
            try:
                purchase_date = opinion.select("span.user-post__published > time:nth-child(2)").pop(0)['datetime']
            except IndexError:
                purchase_date = None
